...
from ScopeFoundryHW.xbox_controller.asi_xbox_controller import ASIXboxMeasure
import logging
...
class XboxApp(BaseMicroscopeApp):
	...
	def setup(self):
		...
		self.stage = self.add_hardware(ASIStageHW(self, name='stage_top',  swap_xy=True))
...
# 		self.xbcontrol_mc = self.add_measurement_component(XboxControllerTestMeasure(self))
		self.xbcontrol_mc = self.add_measurement_component(ASIXboxMeasure(self))
