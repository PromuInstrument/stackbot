...
void printRelays() {
    ...
    for (int i = 0; i < NUM_RELAYS; i++) {
        ...
        Serial.print(delim + RELAYS[i].pulse_width);
...
void handleCmd() {
    ...
    if (INPUT_STRING.length() < 2) { // e.g. 1c=9
    ...
    switch (mode) {
        ...
        case 't':
                  ...
                  relay.pulse_width = (unsigned int)INPUT_STRING.substring(3).toInt();
                  //Serial.println(relay.pulse_width);
                  startPulse(relay);
                  //printRelays();
                  break;
        case '?':
