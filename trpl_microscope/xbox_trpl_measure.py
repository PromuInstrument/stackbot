...
class XboxControllerTRPLMeasure(XboxBaseMeasure):
    ...
    def setup(self):
        ...
        self.settings.New('asi_xy_scale', dtype=float, initial=0.1, unit='mm', ro=False)
        self.stageHW = self.app.hardware['asi_stage']
        #self.shutterHW = self.app.hardware['shutter_se']
        
        self.set_specific_key_map()
        
    def set_specific_key_map(self):
        self.controller.settings.Axis_0.add_listener(
            lambda: self.move_stage_cont(stage_axis='x', controller_axis = 0))
        self.controller.settings.Axis_1.add_listener(
            lambda: self.move_stage_cont(stage_axis='y', controller_axis = 1))
...
#         self.controller.settings.B.add_listener(lambda: self.move_stage_delta(stage_axis='z', delta=-move_delta))
    ...
    def move_stage_cont(self, stage_axis, controller_axis):
        delta = self.controller.settings['Axis_{:d}'.format(int(controller_axis))]*self.settings['asi_xy_scale']
        self.move_stage_delta(stage_axis,delta)
        time.sleep(0.1)
    
    def move_stage_delta(self, stage_axis, delta):
        self.stageHW.settings['{}_target'.format(stage_axis)] += 1.0*delta
